// Taken from https://unpkg.com/houdini-noise@0.0.5/dist/worklet.js
if (typeof registerPaint !== 'undefined') {
    class Noise {
      static get inputProperties() {
        return ['--noise', '--noise-cell-size', '--noise-hue', '--noise-saturation', '--noise-lightness'];
      }
      ...
      paint(ctx, size, properties) {
        ...
        const getSaturation = SATURATION_RANGE.length > 1 ? () => this.getRandom(SATURATION_RANGE[0], SATURATION_RANGE[1]) : () => SATURATION_RANGE[0];
        const LIGHTNESS_RANGE = LIGHTNESS.split(' ').filter(v => v.trim() !== '').map(v => parseInt(v, 10));
        const getLightness = LIGHTNESS_RANGE.length > 1 ? () => this.getRandom(LIGHTNESS_RANGE[0], LIGHTNESS_RANGE[1]) : () => LIGHTNESS_RANGE[0];
